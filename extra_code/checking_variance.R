...
# Ts
# l <- compute_mean(c = c, g_beta = g_beta, pi = pi, m_beta = m_beta)
# m_Ts <- sapply(X = seq(1, B), FUN = function(i) {
#  p_hat <- as.integer(beta_hats$p_hat_mat[,i])
#  m <- dat$m[,i]
#  Ts <- p_hat * (m - l * p_hat)
#  return(c(mean = mean(Ts), var = var(Ts)))
# })
# mean(m_Ts["mean",]) # mean approx zero
# median(m_Ts["var",])
# compute_variance(c = c, g_beta = g_beta, pi = pi, m_beta = m_beta)